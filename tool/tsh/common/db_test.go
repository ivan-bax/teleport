...
func TestDatabaseListHeadless(t *testing.T) {
	...
	require.EventuallyWithT(t, func(t *assert.CollectT) {
		dbs, err := server.GetAuthServer().GetDatabaseServers(ctx, apidefaults.Namespace)
		require.NoError(t, err)
		require.Len(t, dbs, 1)
		require.Equal(t, "test-db", dbs[0].GetName())
	}, 10*time.Second, 100*time.Millisecond, "database should be registered")
